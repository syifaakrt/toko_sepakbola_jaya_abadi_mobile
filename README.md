...
Pertama, saya perlu memastikan bahwa model Product wajib memiliki relasi ForeignKey ke model User bawaan Django untuk mencatat kepemilikan. Sistem autentikasi Django kemudian diekspos sebagai API dengan membuat view khusus untuk registrasi, login, dan logout yang memproses data JSON, bukan HTML. View login (login_user) diatur untuk mengembalikan respons JSON berisi status sukses dan secara otomatis mengatur cookie sesi HTTPOnly.

Pada Flutter, proses autentikasi ditangani menggunakan package seperti pbp_django_auth yang menyediakan CookieRequest. Halaman Registrasi dan Login dibuat menggunakan Form dan TextFormField. Saat submit, fungsi seperti request.login() dan request.register() dipanggil untuk mengirim data POST ke endpoint Django. CookieRequest secara otomatis menangani dan menyimpan cookie sesi yang diterima dari Django. Status autentikasi ini (seperti request.loggedIn dan request.user) dikelola secara global menggunakan Provider, sehingga widget lain, seperti drawer navigasi, dapat secara dinamis menampilkan menu yang berbeda untuk pengguna yang sudah login versus tamu.

Untuk menampilkan daftar semua item, sebuah endpoint JSON publik (misalnya /json/) dibuat di Django ProductEntry.objects.all(). Di Flutter, sebuah halaman (ProductEntryListPage) dibuat menggunakan FutureBuilder yang memanggil fungsi fetchproduct. Fungsi ini menggunakan request.get() untuk mengambil data dari endpoint /json/ tersebut. Data JSON yang diterima di-parsing menjadi List<ProductEntry> menggunakan metode factory ProductEntry.fromJson(Map<String, dynamic> json). ListView.builder kemudian digunakan untuk menampilkan data ini sebagai card yang berisi atribut-atribut yang diminta seperti name, price, dan description.

Pembuatan halaman detail diimplementasikan dengan menambahkan handler onTap pada setiap card di ListView.builder. Handler ini mengeksekusi Navigator.push, mengirimkan objek ProductEntry yang spesifik ke widget ProductDetailPage sebagai argumen constructor. Halaman detail ini kemudian hanya perlu menampilkan semua atribut dari objek yang diterima (termasuk thumbnail, category, dll.). Sebuah tombol, seperti BackButton di AppBar atau tombol kustom yang memanggil Navigator.pop(context), ditambahkan untuk fungsionalitas kembali ke halaman daftar item.

Implementasi filter item berdasarkan pengguna diselesaikan murni di backend untuk efisiensi dan keamanan. Endpoint baru yang terproteksi (misalnya /my-products/) dibuat di urls.py Django. View yang menangani endpoint ini dihiasi dengan decorator @login_required. Di dalam view, queryset difilter menggunakan ProductEntry.objects.filter(user=request.user), di mana Django secara otomatis mengidentifikasi request.user dari cookie yang dikirim oleh CookieRequest Flutter. Di Flutter, halaman ProductEntryListPage yang sudah ada digunakan kembali dengan menambahkan parameter bool showMyProductsOnly. Fungsi fetchproduct di Flutter lantas menggunakan if (showMyProductsOnly) untuk menentukan apakah harus memanggil endpoint /json/ (semua item) atau /my-products/ (item terfilter).
